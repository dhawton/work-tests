...
if [[ "$version" == "latest" ]]; then
  version=$(gcloud container get-server-config --zone us-central1-c --format json | jq -r '.validMasterVersions[0]')
fi

gcloud container clusters create "$name" --cluster-version "$version" --disk-size 100 \
  --num-nodes=$nodes --machine-type=$ntype --zone=us-central1-c --network daniel-vpc \
